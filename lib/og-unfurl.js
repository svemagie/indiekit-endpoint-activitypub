...
/**
 * Trusted publication hostname permitted to resolve to a private LAN address.
 * @param {object} collections - MongoDB collections (carries _publicationUrl)
 * @returns {string|undefined} hostname or undefined
 */
function ownHostFrom(collections) {
  try {
    return collections?._publicationUrl
      ? new URL(collections._publicationUrl).hostname
      : undefined;
  } catch {
    return undefined;
  }
...
export async function fetchAndStoreQuote(collections, uid, quoteUrl, ctx, documentLoader) {
  try {
    // Own publication host may resolve to its LAN address — a remote can
    // legitimately quote one of our own posts by its svemagie.net URL.
    const ownHost = ownHostFrom(collections);
    const object = await lookupWithSecurity(ctx, new URL(quoteUrl), { documentLoader, ownHost });
    if (!object) return;
    ...
    if (!quoteData.author.photo && quoteData.author.url) {
      try {
        const actor = await lookupWithSecurity(ctx, new URL(quoteData.author.url), { documentLoader, ownHost });
        if (actor) {
